...
class file_operation(database):   
    ...
    def writing_image(host_path, parsed_json, i):
        ...
        image_file = open(os.path.join(public_htpath, host_path, str(parsed_json["imgs_names"])[i]), 'wb')
    ...
    def writing_cover(host_path, parsed_json, cover_name):
        sema.acquire()
        cover_file = open(os.path.join(public_htpath, host_path, cover_name), 'wb')
